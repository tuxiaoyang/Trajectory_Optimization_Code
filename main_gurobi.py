...
from output_info import organize_xk, organize_uk, create_csv, create_acmi, get_vio_percentage
from plot_picture import plot_trajectory, plot_results
from main_gurobi_initial import initial_traj
...
# Initial approximate solution: (from the initial solution of the first three iterations of the deterministic problem)
iter_initial = 3
xk_all, uk_all, time_initial = initial_traj(iter_initial)
xk = xk_all[-1]
uk = uk_all[-1]
O1, RO1, std1, O2, RO2, O2_else, std2, v_obs_psi, O1_samples, O2_samples = initialize_obstacles(S, N, num_obs, uk[-1][-1])
...
# 0-1 variables
escape = model.addVars(N , S, vtype=GRB.BINARY, name="escape")

# Control parameter absolute value auxiliary variables
...
for i in range(N + 1):
    Nx[i].start = uk[i][0]
    Nh[i].start = uk[i][1]
    N_phi[i].start = uk[i][2]
    xp[i].start = xk[i][1]
    yp[i].start = xk[i][2]
    hp[i].start = xk[i][0]
    v[i].start = xk[i][3]
    theta[i].start = xk[i][4]
    psi[i].start = xk[i][5]
    phi[i].start = xk[i][6]
    tp[i].start = xk[i][7]
    for j in range(8):
        x_det[i, j].start = xk[i][j]
    for j in range(4):
        u_det[i, j].start = uk[i][j]
t_total.start = uk[-1][3]
...
sgm=1/(1+e**((iter_initial+l1)/l2-l3))
delta = delta_0 * sgm  # Trust region radius
for i in range(N + 1):
...
# Control parameter absolute value control
for i in range(N+1):
...
for i in range(N):
    ...
    for s in range(S):
        ...
        model.addConstr(Mpc * escape[i,s] + expr >= RO1 - sd_obs1 + d_safe, name=f"escape_obs1_{i}_{s}")
...
for i in range(N):
    ...
    for s in range(S):
        ...
        expr = gp.LinExpr()
        expr += nabla[0] * (xp[i] - p_i_k[0]) + nabla[1] * (yp[i] - p_i_k[1]) + nabla[2] * (hp[i] - p_i_k[2])
        model.addConstr(Mpc * escape[i,s] + expr >= RO2 - sd_obs2 + d_safe, name=f"escape_obs2_{i}_{s}")

for i in range(N):
    model.addConstr(1/S*gp.quicksum(escape[i, s] for s in range(S)) - eps <= slack[i], name=f"escape_obs_CC_{i}")
...
obj = t_total\
    + cv * gp.quicksum(slack[i] for i in range(N))\
...
while iter < 30:
    model.optimize()
    iter += 1
    print(f"Iteration {iter + iter_initial}, Objective Value: {model.ObjVal}, t_total: {t_total.X}, Time: {model.Runtime:.2f} seconds")
    ...
    flag = True
    for i in range(N + 1):
        if abs(xk[i][0] - xk_all[-2][i][0]) > epsilon[0] or \
           abs(xk[i][1] - xk_all[-2][i][1]) > epsilon[1] or \
           abs(xk[i][2] - xk_all[-2][i][2]) > epsilon[2] or \
           abs(xk[i][3] - xk_all[-2][i][3]) > epsilon[3] or \
           abs(xk[i][4] - xk_all[-2][i][4]) > epsilon[4] or \
           abs(xk[i][5] - xk_all[-2][i][5]) > epsilon[5] or \
           abs(xk[i][6] - xk_all[-2][i][6]) > epsilon[6] or \
           abs(xk[i][7] - xk_all[-2][i][7]) > epsilon[7]:
            flag = False
    if flag:
        break
...
#     model.computeIIS()
#     model.write("model.ilp")  # Optional: write IIS to file
...
create_csv(N, iter, xk_all, uk_all)
